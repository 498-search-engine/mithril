...
TermNOT::TermNOT(const std::string& index_path, const std::string& term, data::docid_t max_docid)
    : term_reader_(std::make_unique<TermReader>(index_path, term)), 
      max_docid_(max_docid), 
      current_docid_(0) {
    // Start at document ID 0
    advanceToNextNonMatchingDoc();
}

bool TermNOT::hasNext() const {
    return current_docid_ <= max_docid_;
}

void TermNOT::moveNext() {
    if (!hasNext()) {
        return;
    }
...
data::docid_t TermNOT::currentDocID() const {
    if (!hasNext()) {
        throw std::runtime_error("No current document");
    }
    return current_docid_;
}

void TermNOT::seekToDocID(data::docid_t target_doc_id) {
    if (target_doc_id > max_docid_) {
        current_docid_ = max_docid_ + 1; // Set to beyond max to indicate end
        return;
    }
    
    current_docid_ = target_doc_id;
    advanceToNextNonMatchingDoc();
}

void TermNOT::advanceToNextNonMatchingDoc() {
    if (current_docid_ > max_docid_) {
        return;  // Already at end
    }

    // Position term_reader_ at or after our current position
    term_reader_->seekToDocID(current_docid_);
    
    // If term_reader_ is exactly at our current position, we need to skip this document
    while (term_reader_->hasNext() && term_reader_->currentDocID() == current_docid_) {
        current_docid_++;
        if (current_docid_ > max_docid_) {
            return;  // Reached end
        }
...
uint32_t TermNOT::currentFrequency() const {
    // For NOT operators, frequency doesn't really apply
    // Returning 1 as a default value
    return 1;
}

bool TermNOT::hasPositions() const {
    // NOT operators don't have positions
    return false;
}

std::vector<uint32_t> TermNOT::currentPositions() const {
    // NOT operators don't have positions
    return {};
}
