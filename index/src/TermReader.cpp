...
bool TermReader::findTerm(const std::string& term) {
    ...
    for (uint32_t i = 0; i < term_count; i++) {
        ...
        if (current_term == term) {
            ...
            std::cout << "Term has " << postings_size << " postings and " << sync_points_size << " sync points" << std::endl;
            ...
            std::cout << "Successfully loaded term with " << postings_.size() << " postings and " << positions_count_ << " positions" << std::endl;
            return true;
        }
...
std::vector<uint32_t> TermReader::loadPositions(size_t posting_index) const {
    ...
    size_t end_idx = (posting_index == postings_.size() - 1) ? all_positions.size() : (posting_index + 1) * positions_per_posting;
