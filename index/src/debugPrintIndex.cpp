...
public:
    ...
    bool load_documents() {
        ...
        if (verbose_) {
            for (size_t i = 0; i < std::min<size_t>(10, documents_.size()); ++i) {
                std::cout << "  " << documents_[i].id << ": " << documents_[i].url << " - " 
                         << documents_[i].title << std::endl;
            }
    ...
    void print_statistics() {
        ...
        if (!stats_.position_deltas.empty()) {
            double avg_delta = std::accumulate(stats_.position_deltas.begin(), stats_.position_deltas.end(), 0.0) / stats_.position_deltas.size();
            auto max_delta = *std::max_element(stats_.position_deltas.begin(), stats_.position_deltas.end());
            
            std::cout << "  Average delta between positions: " << std::fixed << std::setprecision(2) << avg_delta << std::endl;
            std::cout << "  Largest delta: " << max_delta << std::endl;
            std::cout << "  Compression ratio: " << std::fixed << std::setprecision(2) << compression_ratio << "x" << std::endl;
        }
        
        // Print top terms by document frequency
        std::cout << "\nTop 20 terms by document frequency:" << std::endl;
        std::cout << std::left << std::setw(20) << "Term" 
                  << std::setw(12) << "Doc Freq" 
                  << std::setw(12) << "Term Freq" 
                  << "Positions" << std::endl;
        std::cout << std::string(50, '-') << std::endl;
        
        for (size_t i = 0; i < std::min<size_t>(20, stats_.term_stats.size()); ++i) {
            const auto& term_stat = stats_.term_stats[i];
            std::cout << std::left << std::setw(20) << term_stat.term 
                      << std::setw(12) << term_stat.doc_freq 
                      << std::setw(12) << term_stat.total_term_freq 
                      << term_stat.positions_size << std::endl;
        }
        ...
        for (size_t i = 0; i < stats_.term_freq_dist.size(); ++i) {
            if (stats_.term_freq_dist[i] > 0) {
                ...
                std::string range;
                if (i == 1) range = "1";
                else if (i <= 10) range = "2-10";
                else if (i <= 100) range = "11-100";
                else if (i <= 1000) range = "101-1,000";
                else if (i <= 10000) range = "1,001-10,000";
                else range = ">10,000";
        ...
        for (const auto& pair : grouped_dist) {
            std::cout << std::left << std::setw(10) << pair.first << "| " 
                      ...
                      << (100.0 * pair.second / sum) << "%)" << std::endl;
        }
